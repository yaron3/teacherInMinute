...
let questionState: string | undefined;
...
jest.mock("firebase-functions/v2/tasks", () => ({
  onTaskDispatched: (handler: unknown) => handler,
}));
...
describe("rateTeacher", () => {
  beforeEach(() => {
    ...
    ratingsCollectionRef = {
      ...
      doc: (id: string) => ({ path: `teachers/teacher-1/ratings/${id}` }),
    };
    teacherRef = {
    ...
    txGet.mockImplementation(async (ref: { path: string }) => {
