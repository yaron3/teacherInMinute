...
const ACCEPTED_AT_MS = 1700000000000;
function runLesson(durationSeconds) {
    const endedAtMs = ACCEPTED_AT_MS + durationSeconds * 1000;
    const billing = (0, billing_1.calculateBilling)(ACCEPTED_AT_MS, endedAtMs, COST_PER_MINUTE, COMMISSION_RATE);
    const studentRemainingMinutes = Math.round((INITIAL_STUDENT_MINUTES - billing.minutesToCharge) * 100) / 100;
    return Object.assign(Object.assign({}, billing), { studentRemainingMinutes });
}
...
describe("Edge cases", () => {
    ...
    test("endedAt before acceptedAt yields 0 s (no negative charge)", () => {
        const billing = (0, billing_1.calculateBilling)(ACCEPTED_AT_MS, ACCEPTED_AT_MS - 5000, // 5 s before accepted — shouldn't happen but guard is there
        COST_PER_MINUTE, COMMISSION_RATE);
